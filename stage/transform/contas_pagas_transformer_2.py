...
def _parse_date(series: pd.Series, coluna: str) -> pd.Series:
    """Tenta ISO 8601 (API) e DD/MM/YYYY (CSV manual)."""

    if coluna == "data_de_cadastro":
        return (
            pd.to_datetime(
                series,
                format="ISO8601",
...
def executar(input_dir: Path = INPUT_DIR, output_dir: Path = OUTPUT_DIR) -> None:
    ...
    df["valor_liquido"] = df["valor_liquido"].fillna(
        df["saldo_em_aberto"]
    )
    ...
    _dedup_path = output_dir / "dim_titulo_obra_dedup.csv"
    if _dedup_path.exists():
        _dedup_join = (
            pd.read_csv(_dedup_path, sep=";")[["titulo", "cod_obra", "obra"]]
        ...
        print("  dim_titulo_obra_dedup.csv nao encontrado - enriquecimento de obra ignorado.")
        fato["cod_obra"] = fato["cod_centro_de_custo"]
        fato["obra"] = fato["centro_de_custo"]

    _conta_cc_path = REFERENCE_DIR / "dim_conta_cc.xlsx"

    dim_conta_cc = pd.read_excel(_conta_cc_path, sheet_name="conta_cc")
    ...
    _doc_upper = fato["sigla_documento"].fillna("").str.strip().str.upper()
