...
df_titulo = pd.read_csv(INPUT_DIR / 'titulos.csv', sep=';')
dim_credor_receita = pd.read_csv(INPUT_DIR / 'credores.csv', sep=';')

df_sienge = pd.merge(df_titulo, dim_credor_receita, left_on='credor', right_on='cod_credor', how='left')
...
def ler_auxiliar_empresas():
    df_empresa = pd.read_excel(
        REFERENCE_DIR / 'auxiliar_empresas.xlsx',
        skiprows=3
    )
...
def _preparar_giss(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['_num_doc'] = pd.to_numeric(df['nfs'], errors='coerce').astype(str)
...
df_matched = df_matched[
    [
        ...
        'cnpj_sienge', 'documento_sienge', 'emissao_nf_sienge', 'valor_bruto_sienge', 'score_similaridade',
        'score_label'
    ]
]
